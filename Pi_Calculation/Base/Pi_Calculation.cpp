...
int main()
{
	...
	std::cout << " Время выполнения OpenMP: " << std::chrono::duration_cast<std::chrono::microseconds>(endParOMP - startParOMP).count() << " (микросекунды)" << std::endl;
	std::cout << "Полученный ответ: " << answerParOMP << std::endl;
	std::cout << "Погрешность: " << abs(pi - answerParOMP) << std::endl;

	//Cpp_threads
	std::pair<int, int>* ind_array = new std::pair<int, int>[numThreads]; // Массив индексов
	...
	std::thread* threadArr = new std::thread[numThreads - 1];           //Создание потоков
	...
	std::cout << " Время выполнения Cpp threads: " << std::chrono::duration_cast<std::chrono::microseconds>(endParCppPThr - startParCppPThr).count() << " (микросекунды)" << std::endl;
